...
gen_book_of_life <- function(
  ...
  f_R <- function(d, a, m, u_R) {
    val <- if_else(m, lambda_0 + a * lambda_A + d * lambda_black, na_dbl)
    return(u_R <= val)
  }
  
  f_Y <- function(z, m, r, x, u_Y) {
    val <- if_else(m, beta_0 + x * beta_X + r * beta_R + z * beta_black, na_dbl)
    return(u_Y <= val)
  }
  ...
  book_of_life <- tibble(
      ...
      M_w = f_M(0, A, U_M),
      M_b = f_M(1, A, U_M),
      Y_w_1 = f_Y(0, rep(TRUE, pop_size), R, X, U_Y),
      Y_b_1 = f_Y(1, rep(TRUE, pop_size), R, X, U_Y)
  )
}
```
